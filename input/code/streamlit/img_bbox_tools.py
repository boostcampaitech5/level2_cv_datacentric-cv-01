...
def save_image_with_bbox(img_path,image_id,img_bbox,saving_path=saved_image_folder) -> None:
    # print(image_id)
...
def saving_all(img_path,saving_path=saved_image_folder)->None:
    '''
    ...
    for img_id in img_list:
        if os.path.exists(os.path.join(saving_path,img_id)): #이미 저장된 이미지가 있으면 pass
            pass
        else:
            print(f"saving {img_id}..")
            img_bbox = img_df[img_id]
            save_image_with_bbox(img_path,img_id,img_bbox)
            print(f"done")
